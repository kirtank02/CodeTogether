...
function EditorPageContent() {
  ...
  const monacoNamespaceRef = useRef<any>(null);

  // Dispose Monaco editor on unmount or room change
  useEffect(() => {
    return () => {
      if (monacoEditorRef.current && typeof monacoEditorRef.current.dispose === 'function') {
  ...
  const toggleDarkMode = () => {
    ...
    const newTheme = isDarkMode ? "vs-light" : "vs-dark";
    setTheme(newTheme);

    // Update monaco editor theme using Monaco namespace
    if (monacoNamespaceRef.current) {
      try {
        monacoNamespaceRef.current.editor.setTheme(newTheme);
      } catch (e) {
        monacoNamespaceRef.current.editor.setTheme("onedarkpro");
      }
    }
        ...
        >
          <div className="flex items-center space-x-4">
            ...
            <motion.div
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              initial={{ backgroundColor: "rgba(22, 163, 74, 0.0)" }}
              animate={{
                backgroundColor: isSidebarOpen ? "rgba(22, 163, 74, 0.2)" : "rgba(22, 163, 74, 0.0)"
              }}
              transition={{ duration: 0.3 }}
            ...
            >
              <MonacoEditor
                ...
                editorDidMount={(editor: any, monaco: any) => {
                  ...
                  monacoNamespaceRef.current = monaco;
                }}
              />
              {isLoading && (
